...
namespace ConvertDatabase.WinApp.ConvertClass
{
    ...
    //
    public class ConvertBargh : ISepidar
    {
        string FilePath = @"\Files\Bargh\";
        string branchId = "spncY6QGW2vA7MHJpuQGWa";
        FileRepository fileRepository;

        public ConvertBargh(ConvertDto convertDto)
        {
            fileRepository = new FileRepository(convertDto.SourceData, convertDto.TargetData);
        }
        ...
        public void CreateJournal()
        {
            ...
            foreach (DataRow e in journalData.Rows)
            {
                ...
                journal.fiscalPeriodId = fileRepository.destinationData.FiscalPeriodList.FindLast(a => a.oldId == e["FiscalYearRef"].ToString()).id;
        ...
        private string GetCodeHesab(string accoundId)
        {
            var query = $"WITH category AS ("
                       + "  select AccountId, CAST(Code AS VARCHAR(MAX)) AS Code  FROM[Sepidar01].[ACC].[Account] where ParentAccountRef = -5"
                       + "   UNION ALL"
                       + "    select a.AccountId,CAST(category.Code + a.Code  AS VARCHAR(MAX)) AS Code "
                       + "    FROM[Sepidar01].[ACC].[Account] a,category where a.ParentAccountRef = category.AccountId)"
